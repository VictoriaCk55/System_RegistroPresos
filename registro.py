...
def crear_formulario_registro(root):
    ...
    root.registro_window.title("Registrar Detenido")
    root.registro_window.state('zoomed')
    root.registro_window.configure(bg="#f0f0f0")
    ...
    main_frame = tk.Frame(root.registro_window, bg="#f0f0f0")
    main_frame.pack(expand=True)

    formulario_frame = tk.Frame(main_frame, bg="#ffffff", padx=30, pady=30, relief="groove", bd=2)
    formulario_frame.pack(expand=True, pady=(50, 100))  # Espacio inferior extra
    ...
    def actualizar_entrada_unidad_personalizada(event=None):
        ...
        if seleccion == "Otros":
            # Asegúrate de que esté en la fila correcta (ajusta `unidad_row`)
            unidad_personalizada_entry.grid(row=unidad_row, column=2, pady=5, sticky="w")
        
            # Borra cualquier texto anterior para evitar confusión
            unidad_personalizada_entry.delete(0, "end")
        
            # Usa 'after' para enfocar luego del evento de selección, lo cual mejora el enfoque
    ...
    for idx, campo in enumerate(campos):
        ...
        label = tk.Label(formulario_frame, text=campo + ":", bg="#ffffff", anchor="w", font=("Arial", 11))
        ...
        if campo == "Unidad":
            ...
            unidad_personalizada_entry.grid(row=fila + 1, column=col + 1, sticky="w", pady=5)
            unidad_personalizada_entry.grid_remove()
            unidad_row = fila
    ...
    btn_imagen = tk.Button(formulario_frame, text="Seleccionar Imagen", command=seleccionar_imagen, bg="#2196F3", fg="white", width=20)
    ...
    btn_limpiar = tk.Button(boton_frame, text="Limpiar", command=limpiar_formulario, width=18, height=2)
    ...
    label_carcancho = tk.Label(root.registro_window, image=carcancho, bg="#f0f0f0")
    label_carcancho.image = carcancho
    label_carcancho.place(relx=1.0, rely=1.0, anchor="se", x=-10, y=-70)  # Subido un poco desde abajo

    barra_resized = barra_original.resize((root.registro_window.winfo_width(), 50))
    barra_resized = ImageTk.PhotoImage(barra_resized)
    label_barra = tk.Label(root.registro_window, image=barra_resized, bg="#f0f0f0")
